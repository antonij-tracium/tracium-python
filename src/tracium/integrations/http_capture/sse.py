...
def _max_capture_bytes() -> int:
    """Per-response byte cap for accumulated text/tool-call payloads.

    Set ``TRACIUM_HTTP_CAPTURE_MAX_BYTES`` to override (0 disables the cap).
    Default 1 MiB — large enough for typical chat completions, small enough
    to avoid OOMs on multi-MB image generation or 100k-token streams.
    """
    raw = os.environ.get("TRACIUM_HTTP_CAPTURE_MAX_BYTES")
    if raw is None:
        return 1_048_576
    try:
        value = int(raw)
...
class SSEAccumulator:
    ...
    def __init__(self) -> None:
        ...
        # Soft cap on accumulated text + tool-call arguments to avoid
        # multi-MB memory blow-ups on long streams. Usage / model are still
        # captured after the cap is hit so token counts remain accurate.
        self._text_bytes: int = 0
        self._max_bytes: int = _max_capture_bytes()
        self._truncated: bool = False

    def _capacity(self) -> int:
        """Remaining bytes before the cap. 0 means capped; negative means
        the cap is disabled (no limit)."""
        if self._max_bytes <= 0:
            return -1
        return max(0, self._max_bytes - self._text_bytes)
    ...
    def _append_tool_arguments(self, tc: dict[str, Any], delta: str) -> None:
        if not delta:
            return
        cap = self._capacity()
        ...
        if cap == 0:
            self._truncated = True
            return
        encoded_len = len(delta.encode("utf-8", errors="replace"))
        if encoded_len <= cap:
            fn["arguments"] = existing + delta
            self._text_bytes += encoded_len
            return
    ...
    def finalize(self) -> dict[str, Any]:
        ...
        if self._truncated:
            content = content + "…[truncated by tracium http_capture]"
    ...
    def _merge_tool_call(self, tc: dict[str, Any]) -> None:
        ...
        if isinstance(fn, dict):
            ...
            if isinstance(fn.get("arguments"), str):
                self._append_tool_arguments(existing, fn["arguments"])
